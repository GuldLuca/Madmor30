...
public class HomeController {
    ...
    @RequestMapping("/")        // @RequestMapping("/") definerer hvilket request der skal udløse metoden
    ...
    public String getMealList(Model model){
        meals = dbManager.readAllMeals();       //Læser alle måltider i databasen og lægger dem i arraylisten Meals
        model.addAttribute(mealStr, meals);     // Tilføjer Meals til spring Modellen, så info fra meals kan vises i HTML filen
        return "mealMenu";
    }

    // "method = RequestMethod.GET" bruges når der skal sendes informationer fra server til klient
    ...
    public String addMeal(Meal meal){   //Der bliver oprettet et nyt meal objekt gennem HTML formularen, der blive givet som parameter
    ...
    public String getMealDelete(Model model, Meal meal)
    {
        ...
        return "mealDelete";    //Sender brugeren til HTML sider der spørger om de ønsker at slette det valgte måltid
    }
    ...
    {
        for(Meal m : meals)                     // "Meals" traverseres indtil måltidet med det valgte id bliver fundet.
    ...
    public String updateMeal(Meal meal){
        dbManager.updateMeal(meal);     //Gemmer det ændrede måltid, som er overført fra HTML formularen, i databasen
    ...
    public String mealPackageMenu(Model model) {
        customers = dbManager.getAllCustomersWithMealOrders();  //Fylder "Customers" op med kunder der har bestilt måltider
    ...
    public String mealPackageContent(@RequestParam(value = "id", defaultValue = "1") int id, Model model) {
        mealOrders= dbManager.getMealOrdersFromID(id); //Fylder "MealOrders" op med de ordre den valgte kunde har afgivet.
